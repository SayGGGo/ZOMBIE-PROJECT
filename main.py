...
def start_menu():
    global path_full
    global display, steak_sound, god_status, inf_money, speed_boost
    try:
        ...
        if choice == 1:
            ...
            if choice == 1:
                ...
                while True:
                    ...
                    if speed_boost:
                        print(f"{Fore.LIGHTGREEN_EX}[-]{Style.RESET_ALL} 4 - Буст скорости {Fore.GREEN}[ВКЛ]")
                    else:
                        print(f"{Fore.LIGHTGREEN_EX}[-]{Style.RESET_ALL} 4 - Буст скорости {Fore.RED}[ВЫКЛ]")
                    print(f"{Fore.LIGHTGREEN_EX}[-]{Style.RESET_ALL} 5 - Старт")
                    choice = int(input(f"{Fore.LIGHTGREEN_EX}[>]{Style.RESET_ALL} Выбор: "))
                    if choice == 1:
                        if steak_sound:
                            steak_sound = False
...
def draw_hud(screen, player):
    ...
    text = font.render(f"Деньги: ${player.money}", True, YELLOW)
...
class PromptManager:
    def __init__(self, font, display_time=2.0):
        self.font = font
        self.display_time = display_time
        self.active_prompts = []

    def show_prompt(self, x, y, key_image, text, key):
        prompt = {
            "x": x,
            "y": y,
    ...
    def update_and_draw(self, screen, cam_x, cam_y, player_x, player_y, activation_radius=100):
        now = time.time()
        for prompt in self.active_prompts[:]:
            distance = math.sqrt((prompt["x"] - player_x) ** 2 + (prompt["y"] - player_y) ** 2)

            if distance <= activation_radius:
                if not prompt["shown"]:
                    prompt["shown"] = True
                    prompt["start_time"] = now

            if now - prompt["start_time"] > self.display_time or distance > activation_radius:
                self.active_prompts.remove(prompt)
                continue

            screen.blit(prompt["key_image"], (prompt["x"] - cam_x, prompt["y"] - cam_y))
            screen.blit(prompt["text_surface"], (prompt["x"] + 36 - cam_x, prompt["y"] + 6 - cam_y))
...
background = Background(f"{path_full}bg2.png", speed_factor=0.01, scale=2)
...
class Weapon:
    def __init__(self, name, damage, fire_rate, ammo, color, bullet_speed=10, spread=0, auto=False, price=0,
        ...
        self.last_shot = 0
        self.price = price
        self.image_path = f"{path_full}weapons/{path}.png"
        self.image = pygame.image.load(self.image_path).convert_alpha()
        self.sound = f"{path_full}weapons/{path}.ogg"
        self.sound_object = pygame.mixer.Sound(self.sound)
...
class Player:
    def __init__(self, x, y, w, h):
        self.standing_image = pygame.image.load(
            os.path.join(f"{path_full}player", "character_maleAdventurer_side.png")).convert_alpha()
        self.walking_images = [
            pygame.image.load(
                os.path.join(f"{path_full}player", f"character_maleAdventurer_run{i}.png")).convert_alpha() for i in
            range(3)]
        self.jump_image = pygame.image.load(
            os.path.join(f"{path_full}player", "character_maleAdventurer_jump.png")).convert_alpha()
        self.slide_image = pygame.image.load(
            os.path.join(f"{path_full}player", "character_maleAdventurer_slide.png")).convert_alpha()
    ...
    def move(self, platforms):
        ...
        if (move_left or move_right) and keys[pygame.K_SPACE]:
            self.bhop_speed += 0.005
        else:
            self.bhop_speed = 1

        if move_left:
            dx = -self.speed * self.bhop_speed
        if move_right:
            dx = self.speed * self.bhop_speed

        if move_down and keys[pygame.K_LCTRL]:
            self.is_sliding = True
            self.speed = self.slide_speed
            self.current_image = self.slide_image
            self.slide_time = self.slide_duration
        ...
        if self.is_sliding:
            if self.slide_time > 0:
                ...
                self.is_sliding = False
                self.speed = 5
                self.current_image = self.walking_images[0] if self.facing_right else self.walking_images[1]
    ...
    def draw(self, screen, cam_x, cam_y):
        ...
        if self.current_weapon:
            ...
            weapon_image = self.current_weapon.image

            scaled_weapon = pygame.transform.scale(weapon_image,
                                                   (weapon_image.get_width() // 10, weapon_image.get_height() // 10))

            if self.weapon_direction != -1:
                scaled_weapon = pygame.transform.flip(scaled_weapon, True, False)
            else:
                scaled_weapon = pygame.transform.flip(scaled_weapon, True, True)

            rotated = pygame.transform.rotate(scaled_weapon, -math.degrees(self.weapon_angle_smooth))
            ...
            screen.blit(
                rotated,
                weapon_pos - pygame.Vector2(rotated.get_width() / 2, rotated.get_height() / 2) - (cam_x, cam_y - 40)
            )
            ...
            if boss_win:
                ...
                player.y = 0
                pygame.mixer.init()
                sound = pygame.mixer.Sound(f"{path_full}voices/win.ogg")
                sound.play()
                time.sleep(3)
                running = False
                exit()
...
class Enemy:
    def __init__(self, x, y, difficulty=1, visible=250, boss=False):
        if boss:
            self.images_run = [pygame.image.load(f"{path_full}zombie/boss.png").convert_alpha() for i
                               in range(1)]
            self.image_idle = pygame.image.load(f"{path_full}zombie/boss.png").convert_alpha()
            self.image_jump = pygame.image.load(f"{path_full}zombie/boss.png").convert_alpha()
        else:
            self.images_run = [pygame.image.load(f"{path_full}zombie/character_zombie_run{i}.png").convert_alpha() for i
                               in range(3)]
            self.image_idle = pygame.image.load(f"{path_full}zombie/character_zombie_side.png").convert_alpha()
            self.image_jump = pygame.image.load(f"{path_full}zombie/character_zombie_jump.png").convert_alpha()
...
class DialogueManager:
    def __init__(self, dialogue_file):
        ...
        self.selection = 0
        self.dialogue_height = 1000
        self.last_node = None
        self.saved_state = None
        self.sound = 0
        self.weapons = {
            "ak47": Weapon("AK-47", 3, 10, 120, ORANGE, 15, 5, True, 23000, "ak47"),
            "mosina": Weapon("Мосина", 10, 0.5, 10, PURPLE, 20, 0, False, 10000, "mosina"),
            "p90": Weapon("P90", 1, 15, 60, GREEN, 10, 10, True, 15000, "p90"),
            "knife": Weapon("Нож", 3, 1, 999, GRAY, 5, 0, False, 5000, "knife"),
            "minigun": Weapon("Миниган", 10, 25, 99999, RED, 50, 25, True, 100000, "minigun")
        }
    ...
    def play_voice(self):
        ...
        if voice_file:
            try:
                if not self.sound == 0:
                    self.sound.stop()
                self.sound = pygame.mixer.Sound(f"{path_full}voices/{voice_file}")
                self.sound.play()
            except:
                print(f"Не удалось загрузить звук: {path_full}voices/{voice_file}")
    ...
    def draw(self, screen):
        dialogue_rect = pygame.Rect(50, 50, WIDTH - 100, 400)
    ...
    def save_state(self):        self.saved_state = {
...
class NPC:
    ...
    def draw(self, screen, cam_x, cam_y):
        ...
        if self.show_prompt and not self.interacting:
            ...
            font = pygame.font.SysFont("Benzin", 28)
            key_image = pygame.image.load(f"{path_full}keys/keyboard_e.png").convert_alpha()
            prompt_manager.show_prompt(npc.rect.centerx, npc.rect.top - 40, key_image, "Поговорить", "E")
...
if not os.path.exists(f"{path_full}npc_dialogues"):
    os.makedirs(f"{path_full}npc_dialogues")

player = Player(0, 100, 50, 50)
platform_texture = pygame.image.load(f'{path_full}platform.jpg').convert_alpha()
platform_texture = pygame.transform.scale(platform_texture, (60, 20))

platforms = []
platforms.append(pygame.Rect(0, 500, 40000, 225))

for i in range(5):
    platforms.append(pygame.Rect(1000 + i * 80, 500 - i * 60, 60, 20))

platforms.append(pygame.Rect(1400, 200, 250, 20))
for i in range(3):
    platforms.append(pygame.Rect(1675 + i * 35, 200 + i * 25, 60, 20))

for i in range(3):
    platforms.append(pygame.Rect(1675 + i * 35, 400 + i * 30, 60, 20))
platforms.append(pygame.Rect(1300, 400, 400, 20))

platforms.append(pygame.Rect(1800, 250, 200, 20))
for i in range(2):
    platforms.append(pygame.Rect(2000 + i * 50, 250 + i * 70, 60, 20))

npcs = [
    NPC(1600, 75, f"{path_full}npc_dialogues/npc1.json", f"{path_full}npc/character_malePerson_side.png"),
    NPC(1600, 275, f"{path_full}npc_dialogues/npc3.json", f"{path_full}npc/shop.png"),
    NPC(4000, 375, f"{path_full}npc_dialogues/npc4.json", f"{path_full}npc/sec.png"),
]

enemies = []
for i in range(3):
    x = 4500 + i * random.randint(100, 300)
    y = 300 + math.sin(i) * 50
    difficulty = min(5, 1 + i // 5)
    enemies.append(Enemy(x, y, difficulty))

for i in range(5):
    x = 6000 + (i % 10) * random.randint(100, 300)
    y = 450 - (i // 10) * 60
    enemies.append(Enemy(x, y, 3, 350))

for i in range(10):
    x = 7000 + (i % 10) * random.randint(100, 300)
    y = 450 - (i // 10) * 60
    enemies.append(Enemy(x, y, 3, 500))

for i in enemies:
    effects.append(Effect(i.rect.center, f"+{100 * kill_streak}"))

boss = Enemy(8000, 400, 10, 600, True)
boss.health = 20
boss.speed_x = 4
enemies.append(boss)

font = pygame.font.SysFont("Benzin", 28)
prompt_manager = PromptManager(font)
...
while running:
    ...
    for plat in platforms:
        ...
        screen.blit(platform_texture, screen_platform)
        for x in range(screen_platform[0], screen_platform[0] + plat.width, platform_texture.get_width()):
            screen.blit(platform_texture, (x, screen_platform[1]))
    ...
    for enemy in enemies:
        ...
        if player.rect.colliderect(enemy.rect):
            ...
            if player.on_ground and enemy != boss:
                ...
                if distance != 0:
                    ...
                    new_x = player.rect.x + direction_x * knockback
                    new_y = player.rect.y + direction_y * knockback
                ...
                if enemy.health <= 0:
                    ...
                    if kill_streak >= 2:
                        ...
                        if old_sound_name != sound_name:
                            if steak_sound:
                                sound = pygame.mixer.Sound(f"{path_full}steaks/{sound_name}.mp3")
                                sound.play()
    ...
    draw_hud(screen, player)
    prompt_manager.update_and_draw(screen=screen, cam_x=cam_x, cam_y=cam_y,
                                   player_x=player.rect.x, player_y=player.rect.y)
...
class Weapon:
    ...
    def shoot(self, x, y, target_x, target_y, current_time):
        ...
        if self.name == "Миниган":
            font = pygame.font.SysFont("Benzin", 40 + int(100 / 10))
            text = font.render("ВРЕМЯ СИЯТЬ!", True, (255, 50, 50, 100))
            text.set_alpha(100)
            screen.blit(text, (WIDTH // 2 - text.get_width() / 2, HEIGHT // 3))
...
class Enemy:
    def __init__(self, x, y, difficulty=1, visible=250):
...
class DialogueManager:
    def __init__(self, dialogue_file):
        ...
        self.font = pygame.font.SysFont("Arial", 28)
        self.selection = 0
        self.dialogue_height = 50
        self.last_node = None
        self.saved_state = None
        self.sound = 0
        self.weapons = {
            "ak47": Weapon("AK-47", 2, 10, 240, ORANGE, 15, 5, True, 23000),
...
for i in range(random.randint(3, 6)):
    x = random.randint(2500,3000) + i * 150
    y = 300 + math.sin(i) * 50
    enemies.append(Enemy(x, y, 1))

for i in range(random.randint(3, 5)):
    x = random.randint(5000,5500) + (i % 10) * 50
    y = 300 + math.sin(i) * 50
...
boss = Enemy(5400, 100, 10)
boss.health = 20
boss.speed_x = 4
boss.rect = pygame.Rect(5400, 100, 80, 100)
enemies.append(boss)
